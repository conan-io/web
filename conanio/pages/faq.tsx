...
const FaqPage = () => {
  ...
  const faqs = [
    {
      ...
      "answer": (<div className="pb-2">Yes, Conan includes the ability to <Link href="https://docs.conan.io/2/introduction.html#decentralized-package-manager">create and host private Conan repositories</Link> for sharing packages. JFrog’s “Artifactory” application is the premier enterprise repository hosting platform for Conan packages. Artifactory CE for C and C++ is a free edition, which is designed to provide the C and C++ developer community with the ability to create and host Conan repositories at no cost, and with the benefits of maturity and robustness of the Artifactory server application. JFrog also provides Artifactory hosted as a cloud service which Conan repositories can be created and hosted there as well.</div>)
    }, {
  ...
  for (const [i, faq] of faqs.entries()) {
    if ((i % 2) == 1){
      questionsLeft.push(<li className="faq-anchor"><Link href={"#faq-"+i}>{faq.question}</Link></li>)
    } else {
      questionsRight.push(<li className="faq-anchor"><Link href={"#faq-"+i}>{faq.question}</Link></li>)
    }
